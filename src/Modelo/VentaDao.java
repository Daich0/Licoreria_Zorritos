...
public class VentaDao {
    ...
    public void pdfV(int idventa, int Cliente, double total, String usuario) {
        try {
            ...
            tabla.addCell(c3);
            tabla.addCell(c4);
            String product = "SELECT d.id, d.cod_pro,d.id_venta, d.precio, d.cantidad, p.id, p.nombre FROM detalle d INNER JOIN productos p ON d.cod_pro = p.id WHERE d.id_venta = ?";
            try {
